...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const item = content.sections.flatMap((s) => s.items).find((i) => i.id === id)
  const defItem = PAGE_DEFAULTS.formalnie.sections.flatMap((s) => s.items).find((i) => i.id === id)
    ...
    <>
      ...
      <div className="max-w-3xl mx-auto px-4 py-12">
        <div className="mb-8">
          ...
          >
            <ChevronLeft className="w-4 h-4" /> Wróć do Formalnie
          </Link>
        </div>
